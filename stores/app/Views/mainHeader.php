...
<div class="container-lg  products-container">
    <div class="row g-4">
        ...
        <?php
            ...
            // Structure: variants[0] => [0 => variant_object], variants[1] => [0 => variant_object], etc.
                ...
                <div
                    ...
                    <div class="d-flex align-items-start h-100">
                        ...
                        <div class="flex-grow-1 d-flex flex-column h-100">
                            ...
                            <?php if ($product['is_variant'] == 1 && !empty($productVariants)): ?>

                            <div class="mb-2">
                                <select class="form-select form-select qty-select rounded-pill bg-light border-0 px-2"
                                    style="font-size: 0.98rem;">
                                    <?php
                                            // Generate options for the current product using already extracted variants
                                            foreach ($productVariants as $variant):
                                                // Calculate final price with discount
                                                $originalPrice = (float)$variant['price'];
                                                $finalPrice = $originalPrice;
                                                $discPrice = (float)($variant['disc_price'] ?? 0);
                                                $discType = (int)($variant['disc_type'] ?? 0);

                                                if ($discPrice > 0) {
                                                    if ($discType == 1) { 
                                                        // Fixed amount discount (disc_type = 1)
                                                        $finalPrice -= $discPrice;
                                                    } else { 
                                                        // Percentage discount (disc_type = 0)
                                    ...
                                    <option value="<?= $finalPrice ?>" data-measure="<?= esc($variant['measure']) ?>"
                                        ...
                                        data-disc="<?= $discPrice ?>" data-disc-type="<?= $discType ?>"
                            ...
                            <div class="text-start mb-2">
                                <span class="fw-semibold rounded-pill bg-light py-1 d-inline-block"
                            ...
                            <?php if (!empty($variant['disc_price']) && $variant['disc_price'] > 0): ?>
                            <div class="product_offer" data-measure="<?= esc($variant['measure']) ?>"
                                style="<?= $index === 0 ? '' : 'display:none;' ?>">
                                <?= $variant['disc_type'] == 1 ? '-₹' : '' ?>
                                <?= $variant['disc_price'] ?><?= $variant['disc_type'] == 0 ? '%' : '' ?>
                            </div>
                            <?php endif; ?>
                            <?php endforeach; ?>
                            <?php endif; ?>
