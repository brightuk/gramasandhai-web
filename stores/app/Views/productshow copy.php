    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function collapseAll() {
            categoryItems.forEach(li => {
                ...
                li.querySelectorAll('.myoffcanvas-menu-link, .myoffcanvas-submenu-link').forEach(a => a.classList.remove('active'));
            });
        }
        ...
        sidebar.querySelectorAll('.myoffcanvas-menu-link').forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                if (categoryId) {
                    try { if (typeof window.filterByCategory === 'function') window.filterByCategory(categoryId); } catch(_) {}
                    try { if (typeof window.updateSubcategoryVisibility === 'function') window.updateSubcategoryVisibility(categoryId); } catch(_) {}
                }
            });
        });
    });
...
<script>
    ...
    (function() {
        function getVisibleChips(container) {
            return Array.from(container.querySelectorAll('.subcategory-filter-btn')).filter(b => b.style.display !== 'none');
        }
        ...
        window.updateSubcategoryVisibility = function(categoryId) {
            ...
            if (allowed.size > 0) {
                ...
                const catLinkSpan = document.querySelector(".myoffcanvas-menu-link[onclick*=\"" + "filterByCategory('" + categoryId + "')" + "\"] span");
            ...
            chips.forEach(c => { if (c.style.display === 'none') c.classList.remove('active'); });
            showHideScrollButtons(container);
        };
        ...
        (function decorateCategory() {
            ...
            window.filterByCategory = function(categoryId) {
                ...
                if (typeof window.expandSidebarCategoryById === 'function') window.expandSidebarCategoryById(categoryId);
                if (typeof prev === 'function') prev(categoryId); else if (typeof window.filterProducts === 'function') window.filterProducts();
                if (typeof window.updateSubcategoryVisibility === 'function') window.updateSubcategoryVisibility(categoryId);
                // Clear active chip selection on category change
                const cont = document.getElementById('scrollContainer') || document.getElementById('subcategoryContainer');
                if (cont) cont.querySelectorAll('.subcategory-filter-btn').forEach(b => b.classList.remove('active'));
            };
        })();
        ...
        (function decorateSubcategory() {
            ...
            window.filterBySubcategory = function(subcategoryId, buttonElement = null) {
                ...
                if (!buttonElement) {
                    const chip = (document.querySelector('#scrollContainer .subcategory-filter-btn[data-subcategory="' + subcategoryId + '"]')
                        || document.querySelector('#subcategoryContainer .subcategory-filter-btn[data-subcategory="' + subcategoryId + '"]'));
                    if (chip) {
                        const list = document.querySelectorAll('#scrollContainer .subcategory-filter-btn').length
                ...
                window.currentSubcategoryFilter = subcategoryId;
                if (typeof prev === 'function') prev(subcategoryId, buttonElement); else if (typeof window.filterProducts === 'function') window.filterProducts();
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setTimeout(function initializeCategorySelection() {
                ...
                if (sid) {
                    ...
                    if (typeof window.updateSubcategoryVisibility === 'function' && cid) window.updateSubcategoryVisibility(cid);
                } else if (cid) {
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        container.addEventListener('click', function(e) {
            ...
            if (typeof window.filterBySubcategory === 'function') {
                try { window.filterBySubcategory(subId, btn); } catch (_) {}
            } else if (typeof window.filterProducts === 'function') {
                window.currentSubcategoryFilter = subId;
                try { window.filterProducts(); } catch (_) {}
                // Manually mark active if decorators not loaded
                try {
                    container.querySelectorAll('.subcategory-filter-btn').forEach(b => b.classList.remove('active'));
