    ...
    <div class="row">
        ...
        <div class="col-12 col-lg-9 col-xl-9" style="min-height:30vh;">
            ...
            <div class="row gx-3 gy-4" id="productContainer1">
                ...
                <?php endforeach; ?>
                <!-- Lazy loading indicator will be inserted here -->
                <div class="text-center my-4" id="loadproducts">
                    <button id="loadMoreBtn" class="btn btn-primary mx-auto d-inline-block" onclick="loadMoreProducts()" style="display: none;">
                        Load More Products
                    </button>
                </div>
...
const PRODUCTS_PER_PAGE = 16; // Limit of products loaded per batch
let currentPage = 1;
...
function initializeLazyLoading() {
    ...
    // Check if category_id or subcategory_id is present
    const categoryEl = document.getElementById('category_id');
    const subcategoryEl = document.getElementById('subcategory_id');
    const hasInitialFilter = (categoryEl && categoryEl.textContent.trim()) || 
                             (subcategoryEl && subcategoryEl.textContent.trim());
    
    // If no initial filter, load first batch immediately
    // Otherwise, wait for filter initialization to complete
    if (!hasInitialFilter) {
        // Load first batch
        loadMoreProducts();
    }
...
function loadMoreProducts() {
    ...
    productsToLoad.forEach((el, idx) => {
        ...
        if (img) {
            ...
            // Boost priority for first few images
    ...
    const loadProductsDiv = document.getElementById('loadproducts');
    if (loadMoreBtn) {
        loadMoreBtn.style.display = hasMore ? 'inline-block' : 'none';
    }
    if (loadProductsDiv) {
        loadProductsDiv.style.display = hasMore ? 'block' : 'none';
    }
...
function setupInfiniteScroll() {
    ...
    // Also observe the load more button for manual loading
    const loadMoreBtn = document.getElementById('loadMoreBtn');
    if (loadMoreBtn) {
        lazyLoadObserver.observe(loadMoreBtn);
    }
}
...
function updateNoResultsMessage() {
    ...
    const displayedProducts = visibleProducts.filter(el =>
    ...
    if (visibleProducts.length === 0 && !isLoading) {
        ...
        if (loadProducts) loadProducts.style.display = 'none';
    } else {
...
function filterProducts() {
    ...
    if (visibleCount === 0) {
        if (noResults) noResults.classList.remove('d-none');
        if (sidebar) sidebar.classList.remove('d-none');
        if (nextPageContainer) nextPageContainer.style.display = 'none';
    } else {
        if (noResults) noResults.classList.add('d-none');
        if (sidebar) sidebar.classList.remove('d-none');
        // Pagination buttons will be updated by resetPagination
    }
    ...
    setTimeout(function() {
        ...
        function collapseAll() {
            categoryItems.forEach(li => {
                ...
                li.querySelectorAll('.myoffcanvas-menu-link, .myoffcanvas-submenu-link').forEach(a => {
        ...
        function expandCategoryById(categoryId) {
            ...
            categoryItems.forEach(li => {
                ...
                const match1 = onclickVal.includes("filterByCategory('" + categoryIdStr + "')");
                const match2 = onclickVal.includes("filterByCategory(\"" + categoryIdStr + "\")");
                const match3 = onclickVal.match(/filterByCategory\(['"]?(\d+)['"]?\)/);
                const match4 = match3 && match3[1] === categoryIdStr;
                
                if (match1 || match2 || match4) {
                    // Add active class to parent li - this is what CSS uses to show submenu
        ...
        function highlightSubcategoryById(subcategoryId) {
            ...
            const subcategoryIdStr = String(subcategoryId).trim();
            ...
            categoryItems.forEach(li => {
                ...
                subLinks.forEach(a => {
                    ...
                    // Try multiple matching patterns
                    const match1 = onclickVal.includes("filterBySubcategory('" + subcategoryIdStr + "')");
                    const match2 = onclickVal.includes("filterBySubcategory(\"" + subcategoryIdStr + "\")");
                    const match3 = onclickVal.match(/filterBySubcategory\(['"]?(\d+)['"]?\)/);
                    const match4 = match3 && match3[1] === subcategoryIdStr;
                    
                    if (match1 || match2 || match4) {
                        // Add active class to parent li - this is what CSS uses to show submenu
                        li.classList.add('active');
                        // Expand the parent category submenu
                        if (submenu) {
                            submenu.classList.add('active');
                            submenu.style.display = 'block';
                        }
                        // Highlight the parent category link
                        if (catLink) {
                            catLink.classList.add('active');
                        }
        ...
        // Initialize state - collapse all first
        collapseAll();

        let didExpand = false;
        
        // Handle subcategory first (it also expands parent category)
        if (selectedSubcategoryId) {
            didExpand = highlightSubcategoryById(selectedSubcategoryId);
            
            // Also use the global function if available (for consistency)
            setTimeout(() => {
                if (typeof window.highlightSidebarSubcategory === 'function') {
                    window.highlightSidebarSubcategory(selectedSubcategoryId);
                }
            }, 100);
            ...
            // Ensure pagination is initialized after filtering
            setTimeout(() => {
                if (typeof resetPagination === 'function') {
                    resetPagination();
                }
        ...
        // Handle category if subcategory didn't expand
        if (!didExpand && selectedCategoryId) {
            const ok = expandCategoryById(selectedCategoryId);
            // Set the category filter first
            if (typeof window !== 'undefined') {
                window.currentCategoryFilter = selectedCategoryId;
            ...
            setTimeout(() => {
                if (typeof resetPagination === 'function') {
                    resetPagination();
                } else if (typeof updateFilteredProducts === 'function' && typeof showPage === 'function') {
        ...
        sidebar.querySelectorAll('.myoffcanvas-menu-link').forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                const isHidden = !parent.classList.contains('active');
                collapseAll();
                if (isHidden) {
...
(function() {
    ...
    function highlightSidebarSubcategory(subcategoryId) {
        const sidebar = document.getElementById('sidebar');
        if (!sidebar || !subcategoryId) return false;
        const items = Array.from(sidebar.querySelectorAll('.myoffcanvas-menu-item'));
        let found = false;
        const subcategoryIdStr = String(subcategoryId).trim();
        ...
        items.forEach(li => {
            ...
            subLinks.forEach(a => {
                ...
                // Try multiple matching patterns
                const match1 = onv.includes("filterBySubcategory('" + subcategoryIdStr + "')");
                const match2 = onv.includes("filterBySubcategory(\"" + subcategoryIdStr + "\")");
                const match3 = onv.match(/filterBySubcategory\(['"]?(\d+)['"]?\)/);
                const match4 = match3 && match3[1] === subcategoryIdStr;
    ...
    window.highlightSidebarSubcategory = highlightSidebarSubcategory;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setTimeout(function initializeCategorySelection() {
            ...
            function safeFilterByCategory(categoryId) {
                ...
                if (typeof window.expandSidebarCategoryById === 'function') {
                    window.expandSidebarCategoryById(categoryId);
                }
                // Update subcategory visibility
                if (typeof window.updateSubcategoryVisibility === 'function') {
                    window.updateSubcategoryVisibility(categoryId);
                }
                // Filter products
                if (typeof window.filterByCategory === 'function') {
                    try {
            ...
            function safeFilterBySubcategory(subcategoryId) {
                ...
                // Highlight sidebar subcategory (this will also expand parent category)
                if (typeof window.highlightSidebarSubcategory === 'function') {
                    window.highlightSidebarSubcategory(subcategoryId);
                }
                // Filter products
                if (typeof window.filterBySubcategory === 'function') {
                    try {
            ...
            if (sid) {
                // Prioritize explicit subcategory - this will expand parent category automatically
                safeFilterBySubcategory(sid);
                // Ensure corresponding sidebar entry is highlighted/expanded
                if (typeof window.updateSubcategoryVisibility === 'function' && cid) {
                    window.updateSubcategoryVisibility(cid);
                }
            } else if (cid) {
