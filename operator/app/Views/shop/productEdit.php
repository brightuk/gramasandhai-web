...
<form action="<?= base_url('shop/'.$shop_id.'/saveProduct') ?>" method="Post" enctype="multipart/form-data">
    ...
    <div id="variantContainer">
            ...
            <div class="row mb-5 variant-row">
                ...
                <div class="col-md-2">
                    ...
                    <input type="number" class="form-control" name="measurement[]" required 
                           value="<?= esc($measure_value) ?>" placeholder="Enter (e.g., 1.5)">
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <input type="number" class="form-control" name="prices[]" placeholder="₹" 
                           value="<?= esc($variant['price']) ?>" required>
                </div>
        ...
        <?php else: ?>
            ...
            <div class="row mb-5 variant-row">
                <div class="col-md-2">
                    <label class="form-label">Measurement <span class="text-danger fs-5">*</span></label>
                    <input type="number" class="form-control" name="measurement[]" required placeholder="Enter (e.g., 1.5)">
                </div>
                ...
                <div class="col-md-2">
                    <label class="form-label">Price <span class="text-danger fs-5">*</span></label>
                    <input type="number" class="form-control" name="prices[]" placeholder="₹" required>
                </div>
        ...
        <div class="col-md-4">
            ...
            <select class="form-select" name="sub_category" id="subCategory">
                ...
                <option value="<?= $subcategory['subcategory_id'] ?>" 
                    ...
                    <?= (isset($product) && $product['subcategory_id'] == $subcategory['category_id'] ? 'selected' : '') ?>
                    style="display:none;">
    ...
    <div class="mb-3">
        ...
        <?php if (isset($product) && !empty($product['main_image'])): ?>
            <img src="<?= $img_url . $product['main_image'] ?>" width="100" class="mt-2">
        ...
        <?php 
            ...
            if (is_array($other_images)):
                foreach ($other_images as $image): ?>
                    <img src="<?= $img_url . $image ?>" width="100" class="mt-2 me-2">
                <?php endforeach; ?>
                
                <input type="hidden" name="existing_other_images" value="<?= htmlspecialchars($product['other_images']) ?>">
            <?php endif; ?>
        <?php endif; ?>
    ...
    <div class="mb-3">
        ...
        <?php if (isset($product) && !empty($product['size_chart'])): ?>
            <a href="<?= $img_url. $product['size_chart'] ?>" target="_blank">View Current Size Chart</a>
...
addVariantBtn?.addEventListener('click', () => {
    ...
    // Show remove button and hide add button in the new row
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (mainCategory) {
        // Trigger change event to show correct subcategories
        mainCategory.dispatchEvent(new Event('change'));
        
        // If editing, set the correct subcategory
        <?php if (isset($product)): ?>
            const subCategory = document.getElementById('subCategory');
            if (subCategory) {
                subCategory.value = '<?= $product["subcategory_id"] ?>';
            }
        <?php endif; ?>
    }
});
